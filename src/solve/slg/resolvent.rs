...
use super::{CanonicalConstrainedSubst, ExClause, Literal, Satisfiable};
...
impl<C: Context> Forest<C> {
    ...
    pub(super) fn resolvent_clause(
        ...
        ex_clause
            ...
            .extend(conditions.into_iter().map(|c| match c {
                Goal::Not(c) => Literal::Negative(C::goal_in_environment(environment, *c)),
                c => Literal::Positive(C::goal_in_environment(environment, c)),
            }));
    ...
    pub(super) fn apply_answer_subst(
        ...
        debug!("selected_goal={:?}", infer.debug_goal(selected_goal));
        ...
        match AnswerSubstitutor::<C>::substitute(
            infer,
