...
use super::{CanonicalLeafGoal, Minimums};
use super::stack::StackDepth;

pub(super) struct SearchGraph {
    indices: HashMap<CanonicalLeafGoal, DepthFirstNumber>,
    nodes: Vec<Node>,
}
...
impl SearchGraph {
    ...
    pub fn lookup(&self, goal: &CanonicalLeafGoal) -> Option<DepthFirstNumber> {
    ...
    pub fn insert(&mut self, goal: &CanonicalLeafGoal, stack_depth: StackDepth) -> DepthFirstNumber {
    ...
    pub fn move_to_cache(
        ...
        cache: &mut HashMap<CanonicalLeafGoal, Fallible<Solution>>,
    ) {
