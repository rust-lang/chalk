...
fn placeholder_ty_flags_correct() {
    ...
    assert_eq!(
        placeholder_ty.data(ChalkIr::default()).flags,
    ...
    let opaque_ty = TyKind::Alias(chalk_ir::AliasTy::Opaque(chalk_ir::OpaqueTy {
        ...
        substitution: chalk_ir::Substitution::from_iter(
            ...
            Some(
                ...
                .intern(ChalkIr::default())
                .cast(ChalkIr::default()),
            ),
        ),
    }))
    .intern(ChalkIr::default());
    assert_eq!(
        opaque_ty.data(ChalkIr::default()).flags,
...
fn dyn_ty_flags_correct() {
    let internal_ty = TyKind::Scalar(chalk_ir::Scalar::Bool).intern(ChalkIr::default());
    ...
    let ty = TyKind::Dyn(dyn_ty).intern(ChalkIr::default());
    assert_eq!(
        ty.data(ChalkIr::default()).flags,
...
fn flagless_ty_has_no_flags() {
    let ty = TyKind::Str.intern(ChalkIr::default());
    assert_eq!(ty.data(ChalkIr::default()).flags, TypeFlags::empty());
    ...
    .intern(ChalkIr::default());
    assert_eq!(fn_ty.data(ChalkIr::default()).flags, TypeFlags::empty());
}
...
fn static_and_bound_lifetimes() {
    let substitutions = chalk_ir::Substitution::from_iter(
        ChalkIr::default(),
        vec![
            chalk_ir::GenericArgData::Lifetime(chalk_ir::LifetimeData::Static.intern(ChalkIr::default()))
                .intern(ChalkIr::default()),
            chalk_ir::GenericArgData::Lifetime(lifetime!(bound 5)).intern(ChalkIr::default()),
        ],
    );
